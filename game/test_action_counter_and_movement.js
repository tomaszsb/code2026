/**
 * Comprehensive Verification: Action Counter & Movement Fixes
 * Consolidates tests for action counter initialization, interaction, and movement integration.
 */

function runAllActionCounterTests() {
    console.log('🔧 COMPREHENSIVE VERIFICATION: Action Counter & Movement Fixes');
    ...
    const results = {
        initialization: testVerification1_Initialization(),
        interaction: testVerification2_Interaction(),
        movementIntegration: testVerification3_MovementAndIntegration()
    };

    // Final summary
    console.log('\n' + '='.repeat(70));
    console.log('📊 VERIFICATION SUMMARY:');
    console.log(`✅ Verification #1 (Initialization): ${results.initialization ? 'PASS' : 'FAIL'}`);
    console.log(`✅ Verification #2 (Interaction): ${results.interaction ? 'PASS' : 'FAIL'}`);
    console.log(`✅ Verification #3 (Movement Integration): ${results.movementIntegration ? 'PASS' : 'FAIL'}`);

    const allPassed = results.initialization && results.interaction && results.movementIntegration;
...
function testVerification3_MovementAndIntegration() {
    console.log('\n📋 VERIFICATION #3: Movement + Action Counter Integration');
    console.log('Testing: Previous movement fix still works and integrates with action counter');
    ...
    try {
        ...
        if (updatedPlayer.position === testDestination) {
            ...
            // Check if spaceActionCompleted was processed and action counter updated
            const finalActionCount = calculateExpectedActionCounter({
                ...simulateDetectSpaceRequirements(updatedPlayer),
                hasRolled: window.ComponentUtils.requiresDiceRoll(updatedPlayer.position, updatedPlayer.visitType || 'First'), // Assume rolled if dice required
                selectedMove: testDestination // Assume move selected
            });
            console.log(`✅ V3 PASS: Final Action Counter: ${finalActionCount.completed}/${finalActionCount.required}`);
            console.log(`✅ V3 PASS: End Turn Should Be Enabled: ${finalActionCount.completed >= finalActionCount.required}`);
...
// Helper functions (from test_action_counter_initialization_fix.js)
...
window.runAllActionCounterTests = runAllActionCounterTests;
...
window.testVerification3_MovementAndIntegration = testVerification3_MovementAndIntegration;

console.log('🧪 Consolidated Action Counter & Movement verification loaded');
console.log('📞 Run: runAllActionCounterTests() for full test suite');
